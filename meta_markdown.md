...
## 회고록 
메타 CEO **마크 저커버그** 가 아이언맨에 나온 AI 비서 "자비스" 를 모방해 집안에서의 비서를 만들어보는 프로젝트를 하고 난 후에 느낀 점을 포함해서 힘들었던 점들을 적어주셨습니다.   
집안 환경을 관장하고 있는 조명, 온도, 가전 등으로 "자비스"는 이들을 통제/제어 할 수 있게 만들었습니다. 사람의 라이프스타일등을 데이터화 및 학습하는데 센서들을 통합하는데에 어려웠고 이미지/영상/음성 등을 인식하는데에 큰 어려움이 있을꺼라 생각합니다. 또한 조명 가전들은 WIFI 등에 연결되어있지 않는 객체들이랑 더 어려웠을꺼고 이 프로젝트를 진행할 시에 IoT 가 큰 역할을 할것이라 생각했습니다. 
**저커버그** 는 많은 기기들을 연결하고 라이스타일을 자유자재로 학습시에 더 시너지가 날것이라 생각하고 이야기를 끝 맞췄습니다.   
<b> </b>
제가 이것을 읽고 느낀 점은 정말 Tech/AI 가 쓰일 수 있는 것들을 다양하구나 느꼈습니다. 
